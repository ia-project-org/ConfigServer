pipeline {
    ...
    environment {
        REPO_URL = 'https://github.com/ia-project-org/ConfigServer.git'
             ...
             steps {
                 script {
                     ...
                     withCredentials([usernamePassword(credentialsId: 'github', passwordVariable: 'GITHUB_PASSWORD', usernameVariable: 'GITHUB_USERNAME')]) {
                         ...
                         sh """
                             git status
                             sed -i 's|image: minaouimh/ai:.*|image: minaouimh/ai:${imageTag}|g' deployment/config-server/config-server-deployment.yaml
